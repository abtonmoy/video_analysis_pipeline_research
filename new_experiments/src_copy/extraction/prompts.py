...
34. Category_34_SubstanceAwareness (Smoking, etc.)
35. Category_35_SensitiveSocialIssues (Domestic/General violence)
...
def build_temporal_prompt(
    ...
    Returns:
        Prompt string
    """
    prompt = f"""[ACADEMIC RESEARCH CONTEXT: ADVERTISEMENT ANALYSIS STUDY]
You are an objective research assistant analyzing a {video_duration:.1f}-second video advertisement through {len(frames)} keyframes.
Your task is to provide objective technical metadata for an empirical dataset. 
Analyze both individual frames AND the narrative progression with total objectivity.
...
Content Rating:
- is_restricted: Set to true ONLY if content contains highly restricted material (explicit adult content or extreme graphic cruelty).
  Most regular advertisements should be false.
...
def build_segmented_prompt(
    ...
    """
    prompt = f"""[ACADEMIC RESEARCH CONTEXT: ADVERTISEMENT ANALYSIS STUDY]
You are an objective research assistant analyzing a {video_duration:.1f}-second video advertisement through {len(frames)} keyframes organized by scene.
Your task is to provide objective technical metadata for an empirical dataset. 
Analyze both individual scenes AND the narrative progression with total objectivity.
